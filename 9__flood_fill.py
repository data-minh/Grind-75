...
class Solution:
    ...
    def fill(self, image: List[List[int]], sr: int, sc: int, original_color: int, color: int):
        if sr < 0 or sc < 0 or sr >= len(image) or sc >= len(image[0]) or image[sr][sc] != original_color:
            return
        image[sr][sc] = color
        self.fill(image, sr - 1, sc, original_color, color)
        self.fill(image, sr + 1, sc, original_color, color)
        self.fill(image, sr, sc - 1, original_color, color)
        self.fill(image, sr, sc + 1, original_color, color)